...
var Difference  = require('../models/difference.js');
...
function verifyWord(difference) {
  var id = difference._id;
  var text = difference.text;
  ...
  return Difference.findByIdAsync(id)
    .then(function(databaseCopy) {
      ...
      if (wordCompareResult) {
        ...
        console.log('Full match'.green);
        return difference;
      } else {
        var ocr1 = databaseCopy.texts[0];
        var ocr2 = databaseCopy.texts[1];

        var ocrL    = new Levenshtein(ocr1, ocr2).distance;
        var pattern = calculateLCS(ocr1, ocr2).toLowerCase();

        var inputVsPattern = new Levenshtein(text, pattern).distance;

        if (inputVsPattern <= (c * ocrL + k)) {
        ...
        console.log();
        console.log('Multiplier                    :', c);
        console.log('Input                         :', difference.text);
        console.log('Pattern                       :', pattern);
        console.log('Allowance                     :', c * ocrL + k);
        console.log('Input vs. Pattern Levenshtein :', inputVsPattern);
      }
      console.log();
    });
}
 ...
 * input.
 */
function closestWord(input, differences) {
  // First return the closest row in the matrix.

  var closestOcr = _.min(differences, function(difference) {

    var ocr1 = difference.texts[0];
    var ocr2 = difference.texts[1];
    var l1 = new Levenshtein(input, ocr1);
    var l2 = new Levenshtein(input, ocr2);
   ...
   */
  var match;
  if (wordCompare(input, closestOcr.ocr1, closestOcr.ocr2)) {
    match = true;
  } else {
  ...
  var result = {
    match: match,
    _id: closestOcr._id,
    ocrMatch: [closestOcr.ocr1, closestOcr.ocr2]
  };
  // console.log(result);
  return result;
}
